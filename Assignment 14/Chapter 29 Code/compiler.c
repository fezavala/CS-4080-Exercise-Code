...
// Now tracks current class id and method names. Used to rename methods to distinguish super class methods
// and child class methods of the same name
...
// New inner parsing that replaces the old super parsing
static void inner(bool canAssign) {
    ...
    if (currentClass != NULL) {
        char name[256];
        // Rename current method name by attacking the id number of the class to the back of it
        sprintf(name, "%.*s@%x",
...
static void classDeclaration() {
    ...
    // Class now needs its id passed to the vm
